...
function Home({ demo, capabilities, spotify, youtube, onRefreshConnections, onDemo, onManual, onImportError, onCompare, onTransfer, onAgent, onDiscover, onExport }: {
  ...
  return <>
    ...
    <section className="connection-section" id="connection-panel">
      <div className="import-guide"><strong>第一次使用：如何导入歌单？</strong><p>Spotify / YouTube：Connect 官方账号 → 在官方页面授权 → 选择歌单 → 读取歌曲并核对预览。</p><p>其他平台：上传文件或复制歌曲列表 → Rust 解析 → 核对预览 → 确认分析。Apple 公开目录也可由部署者配置官方 API 后读取。</p><small>平台 API 曲目用于确认后的传输；体验分析和 Agent 请使用自备文件或文本。</small></div>
