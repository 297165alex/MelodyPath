...
> 💡 **如何准备歌单：不必先到处寻找 CSV**
>
> 推荐流程：**官方 OAuth Connector → 文件导入备用方案 → 文本粘贴快速导入**。
>
> **方式 1：官方连接（推荐）**
>
> Spotify、YouTube / YouTube Music 等支持 OAuth 的平台，可以直接点击 Connect，在平台官方页面授权后读取自己的歌单。
>
> **方式 2：通用文件导入**
>
> 当平台没有开放稳定的官方 API，或者你希望离线使用时，可以上传 CSV、TXT、TSV、JSON、M3U 或 M3U8。没有导出文件也没关系，直接复制歌曲列表，按每行 `歌手 - 歌名` 粘贴即可。
>
> CSV 示例：
>
> ```csv
...
> ```
>
> 对于网易云音乐、QQ音乐、酷狗音乐和汽水音乐：平台能导出文件时可直接上传；不能导出时，复制歌曲列表并整理成上述文本即可。MelodyPath 不需要你的账号密码或 Cookie。
