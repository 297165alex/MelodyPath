...
export default function ExportModal({ initialPlatform, tracks, sourceLabel, statuses, onClose }: Props) {
  ...
  const makePreview = async () => {
    ...
    try { if (platform === 'spotify' && !await spotifyWrite.check(makePreview)) return; setPreview(await api.previewExport(platform, playlistName, chosenTracks, format)) }
    catch (reason) { if (platform === 'spotify') spotifyWrite.failed(reason, makePreview); setError(reason instanceof Error ? reason.message : '无法生成预览') }
  ...
  const execute = async () => {
    ...
    setBusy(true); setError('')
    try { if (platform === 'spotify' && !await spotifyWrite.check(execute)) return; setResult(await api.executeExport(preview.preview_id, selections)) }
    catch (reason) { if (platform === 'spotify') spotifyWrite.failed(reason, execute); setError(reason instanceof Error ? reason.message : '写入失败') }
    finally { setBusy(false) }
  }
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="export-modal" role="dialog" aria-modal="true" aria-label="保存到音乐平台">
      ...
      {platform === 'spotify' && spotifyWrite.prompt}
      {!preview && !result && <>
        <div className="safety-note"><strong>授权边界</strong><span>只创建新歌单。预览与确认之前不会修改任何账号；授权只通过平台 OAuth。</span></div>
        <div className="platform-pills">
          {['spotify', 'apple_music', 'youtube', 'file', 'demo'].map((key) => <button key={key} disabled={spotifyWrite.waiting} className={platform === key ? 'active' : ''} onClick={() => { setPlatform(key); setError('') }}>{platformNames[key]}</button>)}
        </div>
        {status && <div className={`status-strip ${status.availability}`}>
          <span className={`status-dot ${status.authorized ? 'online' : ''}`} />
          <span>{status.message}</span>
          {platform === 'youtube' && status.availability === 'available' && !status.authorized && <a className="mini-button" href={`/api/${platform}/authorize?write=true`}>单独授权歌单写入</a>}
        </div>}
        {platform === 'youtube' && <p className="policy-box">Google 没有仅限歌单写入的 scope；仅需读取时无需升级。写入授权文案也涵盖视频、评论等操作，但本应用只允许用户确认后新建私有歌单及添加视频。</p>}
        <label className="field"><span>新歌单名称</span><input disabled={spotifyWrite.waiting} value={playlistName} maxLength={100} onChange={(event) => setPlaylistName(event.target.value)} /></label>
        ...
        <div className="selection-heading"><div><strong>歌曲预览</strong><span>{chosenTracks.length} / {tracks.length} 首已选择</span></div><button disabled={spotifyWrite.waiting} className="text-button" onClick={() => setSelected(selected.size === tracks.length ? new Set() : new Set(tracks.map((t) => t.id)))}>{selected.size === tracks.length ? '取消全选' : '选择全部'}</button></div>
        <div className="track-picker">
          {tracks.map((track, index) => <label className="pick-row" key={track.id}>
            <input disabled={spotifyWrite.waiting} type="checkbox" checked={selected.has(track.id)} onChange={() => toggle(track.id)} />
        ...
        {error && <p className="error-box">{error}<button onClick={() => void (preview ? execute() : makePreview())}>重试</button></p>}
        <footer className="modal-actions"><button className="secondary" onClick={onClose}>取消</button><button className="primary" disabled={busy || spotifyWrite.waiting || !playlistName.trim() || chosenTracks.length === 0 || status?.availability !== 'available'} onClick={makePreview}>{busy ? '正在匹配…' : '下一步：匹配并预览'}</button></footer>
      </>}
      ...
      {preview && !result && <>
        ...
        <div className="match-list">
          {preview.matches.map((match) => <div className={`match-row ${match.status}`} key={match.source_track.id}>
            ...
            {match.status === 'needs_confirmation' && <select disabled={spotifyWrite.waiting} value={selections[match.source_track.id] ?? ''} onChange={(event) => setSelections({ ...selections, [match.source_track.id]: event.target.value })}>
        ...
        <label className="confirm-check"><input disabled={spotifyWrite.waiting} type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /><span>我已检查以上歌曲与版本，并明确同意创建<strong>新的</strong>“{preview.playlist_name}”歌单。未确认或无法匹配的歌曲不会写入。</span></label>
        {error && <p className="error-box">{error}<button onClick={() => void (preview ? execute() : makePreview())}>重试</button></p>}
        <footer className="modal-actions"><button className="secondary" disabled={spotifyWrite.waiting} onClick={() => { setPreview(null); setConfirmed(false) }}>返回选择</button><button className="primary" disabled={!confirmed || busy || spotifyWrite.waiting} onClick={execute}>{busy ? '正在创建…' : platform === 'file' ? '确认并生成文件' : '确认并创建新歌单'}</button></footer>
      </>}
