...
export default function App() {
  ...
  const activePersonal = dataState === 'DEMO'
    ? demo?.personal ?? null
    : dataState === 'REAL_FILE' || dataState === 'REAL_TEXT' || dataState === 'REAL_PUBLIC_LINK'
      ? manualPersonal
      : null
...
function LinkImportStatus({ result }: { result: PlaylistLinkInspection }) {
  ...
  return <div className="import-stages" aria-label="公开链接读取状态">
    ...
    <p><b>3 · Track Import / 歌曲读取</b><span>{result.platform === 'netease' ? (hasTracks ? `Tracks imported: ${result.preview_tracks.length} · Imported ${result.preview_tracks.length} / ${result.track_count ?? '?'} tracks · 未导入 ${(result.track_count ?? 0) - result.preview_tracks.length} 首，请核对预览。` : 'Playlist recognized but tracks unavailable. 尚未导入歌曲。当前无法通过官方接口读取完整歌曲列表；公开页面未能提供完整、有效曲目。') : hasTracks ? `官方 API 已返回 ${result.track_count ?? result.preview_tracks.length} 首歌曲，请核对预览。` : chinaPlatform ? '尚未导入歌曲。当前无法通过官方接口读取完整歌曲列表。' : '尚未获得可导入歌曲，请按下方提示完成配置、授权或重试。'}</span></p>
...
function Home({ demo, capabilities, spotify, youtube, onRefreshConnections, onDemo, onManual, onImportError, onCompare, onTransfer, onAgent, onVersions, onDiscover, onExport }: {
  ...
  onDemo: () => void
  onManual: (personal: PersonalAnalysis, state: 'REAL_FILE' | 'REAL_TEXT' | 'REAL_PUBLIC_LINK') => void
  onImportError: () => void
  onCompare: () => void
  onTransfer: () => void
  onAgent: () => void
