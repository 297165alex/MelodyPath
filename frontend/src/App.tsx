...
function Home({ demo, capabilities, spotify, youtube, onRefreshConnections, onDemo, onManual, onImportError, onCompare, onTransfer, onAgent, onDiscover, onExport }: {
  ...
  const inspectLink = async () => {
    ...
    catch (reason) { setError(reason instanceof TypeError ? '公开歌单链接检查未能连接后端，请确认服务已启动或使用本地文件导入。' : reason instanceof Error ? `公开歌单链接检查失败：${reason.message}` : '链接检查失败，请使用本地文件导入。') }
  ...
  return <>
    ...
    <section className="connection-section" id="connection-panel">
      <div className="section-heading wide"><span className="eyebrow">OFFICIAL CONNECTIONS FIRST</span><h2>连接你的音乐平台</h2><p>MelodyPath 不要求用户提供账号密码或 Cookie。Spotify / YouTube 使用官方 OAuth；其他平台按下方真实能力导入。公网服务的 Developer Credentials 由部署者在后端配置；Self-host 时由本地部署者配置。</p></div>
      <div className="platform-group"><div className="platform-group-title"><strong>可操作平台</strong><span>能力状态由 Rust 后端实时返回</span></div><div className="platform-grid">
        {capabilities.filter((item) => ['spotify', 'youtube_music', 'apple_music', 'netease', 'qq_music', 'kugou', 'qishui'].includes(item.platform)).map((capability) => {
          ...
          return <article className={`platform-card status-${connected ? 'connected' : capability.capability_status}`} key={capability.platform}><div className="platform-card-head"><span className={`platform-mark mark-${capability.platform}`}>{capability.display_name.slice(0, 1)}</span><div><h3>{capability.display_name}</h3><span className="capability-label">{connected ? `已连接 · ${displayName}` : capability.configured && capability.action_kind === 'connect' ? '已配置，可连接' : capability.status_label}</span></div></div><p>{capability.description}</p><div className="capability-facts"><span>真人验收：{isSpotify || isYoutube ? 'REAL VERIFIED · 读取（既有验收）' : 'NOT REAL VERIFIED'}</span><span>Developer Credentials：{capability.auth_supported || capability.platform === 'apple_music' ? '由部署者配置，普通用户无需提供' : '文件/文本不需要'}</span><span>公开 URL：{capability.public_playlist_links}</span><span>完整曲目导入：{capability.public_link_import_supported ? (capability.configured ? '官方 API（受权限与地区限制）' : '需部署者配置') : '不可通过 URL 读取'}</span><span>账号授权：{capability.auth_supported ? '官方 OAuth' : '未接入'}</span><span>文件/文本：{capability.file_import_supported ? 'FILE IMPORT' : 'NOT AVAILABLE'}</span><span>读取：{humanCapability(capability.playlist_read)}</span><span>写入：{humanCapability(capability.playlist_write)}</span></div>{capability.policy_notice && <small>{capability.policy_notice}</small>}<div className="platform-actions">
            ...
            {capability.platform === 'apple_music' && <button className="secondary" onClick={() => setConfigPlatform('apple')}>Apple 目录配置</button>}{capability.public_link_import_supported && <button className="text-button" onClick={() => document.getElementById('public-link-panel')?.scrollIntoView({ behavior: 'smooth' })}>公开歌单 URL</button>}{capability.official_docs_url && <a className="docs-link" href={capability.official_docs_url} target="_blank" rel="noreferrer">官方说明 ↗</a>}
          </div></article>
        })}
      </div></div>
