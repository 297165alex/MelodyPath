...
function Home({ demo, capabilities, spotify, youtube, onRefreshConnections, onDemo, onManual, onImportError, onCompare, onTransfer, onAgent, onDiscover, onExport }: {
  ...
  return <>
    ...
    <section className="link-section" id="public-link-panel"><div className="section-heading"><span className="eyebrow">PUBLIC PLAYLIST LINK</span><span className="experimental-status">按平台显示实际能力</span><h2>或者粘贴公开歌单链接</h2><p>Spotify / YouTube 授权后通过官方 API 读取；Apple Music 公开目录需部署者配置，尚未真人验收。中国音乐平台：支持公开链接检测与文件/文本导入，不需要账号密码。</p></div><div className="input-card link-card"><label className="field"><span>Spotify、YouTube、Apple Music 或中国平台歌单链接</span><div className="link-input-row"><input id="playlist-link" type="url" placeholder="https://music.163.com/playlist?id=…" value={link} onChange={(event) => { setLink(event.target.value); setLinkResult(null) }} onKeyDown={(event) => { if (event.key === 'Enter') void inspectLink() }}/><button className="primary" disabled={linkBusy || !link.trim()} onClick={() => void inspectLink()}>{linkBusy ? '正在检查…' : '检查并导入预览'}</button></div></label>{linkResult && <div className={`link-result ${linkResult.can_analyze ? 'ready' : ''}`}><div><span className={`access-dot access-${linkResult.access_status}`}/><strong>{linkResult.recognized ? `${linkResult.platform_label} · ${linkResult.playlist_id ? `ID ${linkResult.playlist_id}` : '链接已识别'}` : '未识别链接'}</strong></div><p>{humanPlatformText(linkResult.message)}</p><small>当前能力：{humanCapability(linkResult.capability)} · 歌单 ID：{linkResult.playlist_id_valid ? '格式有效' : '未验证'}</small>{linkResult.capability === 'AUTH_REQUIRED' && (() => {
...
function AgentPage({ currentAnalysis, comparisonBinding }: { currentAnalysis: PersonalAnalysis | null; comparisonBinding: { analysis_a_id: string; analysis_b_id: string } | null }) {
  ...
  const toolCalls = task ? parseJsonArray<{ tool: string; step_id: string; attempt: number }>(task.tool_calls_json) : []
